...
Issue #4当前没有“开始新任务”按钮。pyRevit 6.5.3的`Reload`不会清除这个Revit进程内的暂停锁。若人工测试需要重新建立绑定，必须完全关闭Revit，确认该Revit进程已经退出，然后重新启动Revit并打开指定开发测试副本。新Revit进程中再次点击按钮后，应重新显示`bound / verified / allowed / none`。
